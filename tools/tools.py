import time, json, torch, cv2, copy, numpy as np
...
def annotate_frame(frame, results):
    for result in results : 
        ...
        for j in result["landmarks"]:
            cv2.circle(frame, j, 1, (0,255,0), -1)
            
        cv2.rectangle(frame, (x1,y1), (x2, y2), (0,0,255), 2, cv2.LINE_AA)
        
        if 'class' in result and 'distance' in result:
            labl = f"{result['class'].split('_')[-1]}:{result['distance']:.2f}"
            cv2.putText(frame, labl, (x1,y1-10), cv2.FONT_HERSHEY_COMPLEX,1, (0,255,0), 2, cv2.LINE_AA)
